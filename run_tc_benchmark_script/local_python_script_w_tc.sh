...
python pytorch_collective_test.py --dist-url tcp://"$MASTER_IP":9000 --dist-backend cupy_nccl --use-cuda True --world-size "$WORLD_SIZE" --rank "$RANK" --dim-mb "$DIM" >>  "./logs/${timestamp}_group_cupyNCCL_d${DELAY_MS}b${RATE_GBIT}.log"
